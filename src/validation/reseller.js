...
const validate = {
  create: (request, response, next) => {
    request.assert('revendedor.cpf', 'CPF é obrigatório').notEmpty().trim(); // TODO check regex and algo
    request.assert('revendedor.cpf', 'CPF deve ser numérico').matches(/^[0-9\s]+$/); // TODO check regex and algo
    request.assert('revendedor.cpf', 'CPF deve ser 11 posicoes').isLength({ min: 11, max:11 }); // TODO check regex and algo
  ...
  login: (request, response, next) => {
    request.assert('login.cpf', 'CPF é obrigatório').notEmpty().trim(); // TODO check regex and algo
    request.assert('login.cpf', 'CPF deve ser numérico').matches(/^[0-9\s]+$/); // TODO check regex and algo
    request.assert('login.cpf', 'CPF deve ser 11 posicoes').isLength({ min: 11, max:11 }); // TODO check regex and algo
